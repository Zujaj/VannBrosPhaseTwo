...
import { FIELD_CODE_MAX_LENGTH, PLOT_FORM_LABELS } from '../pages/plot.page';
 ...
 *
 *   AC #1  coordinates optional on Add Plot .............. automated here (TC-1, TC-2)
 *   AC #2  PlotJobField service enabled and functional ... half automated (TC-5); "functional"
 *                                                          is a D365 round-trip, manual
 *   AC #3  web-created plots sync to FinOps .............. web half automated (TC-3); the
 *                                                          FinOps half is manual
 *   AC #4  FinOps project syncs back to the web app ...... manual (starts in D365)
 *
 ...
 *
 * This is the cheap half of AC #1: it asserts the *contract* the criterion states, without
 * writing a plot to the shared QA tenant. TC-2 proves the behaviour behind it.
 */
test('@ADO-26093 the Add Plot form marks every field required except Coordinate', async ({
  ...
  // AC #1: "Ensure coordinates are optional on the Add Plot form".
  expect(
...
/**
 * TC-2 (AC #1, `@mutating`) — a plot saves with the Coordinate box left blank.
 *
 * The real assertion of AC #1: a required-marker check alone would pass while the server
 * still rejected a coordinate-less payload. Creates a real plot on the shared QA tenant and
 * does NOT clean up — see the plan's "Test data" note; delete leftovers with
 * `DELETE /api/Field/{id}`.
 */
test('@mutating @ADO-26093 saves a plot with no coordinates', async ({ plotPage, listPage }) => {
...
/**
 * TC-3 (AC #3, web half) — the Plot grid is the web-side record that FinOps syncs against.
 *
 * AC #3's own assertion lives in D365 (`AgriERP management > Plots/Fields`) and cannot be
 * made from this app. What IS assertable here is the precondition: the grid exposes the
 * identity columns FinOps matches on, populated for every row. A blank `Field Code` would
 * break the sync regardless of the batch job's state, so this is a real guard, not a
 * placeholder.
 */
 ...
 * `code` at 10 (`PlotFieldAPIModel`, vannbrosphasetwo-vann-api-qa skill). The form is expected to
 * enforce that client-side rather than let the server reject the save. Non-mutating: the
...
/**
 * TC-5 (AC #2, "enabled" half) — PlotJobField is registered and running in the Sync Console.
 *
 * AC #2 has two halves. "Enabled" is observable from the web app: the Sync Console's Service
 * Status grid lists every sync service with its `Job Status`. "Functional" means rows
 * actually reach D365 and stays manual.
 *
...
test('@ADO-26093 the PlotJobField sync service is listed and active', async ({
  ...
  expect(
    ...
    `plot sync service "${names[index]}" is not active (AC #2 of ADO #26093)`,
